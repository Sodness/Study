...
public class ServerSend extends JFrame implements ActionListener {
    ...
    int port = 12345;
    StringTokenizer st;
    boolean pw = false;    //준영 : 비밀번호설정 유무 [0 = 비밀번호X, 1 = 비밀번호O]
    ...
    private void Server_start()
    {
        try {
            ...
            JOptionPane.showMessageDialog(null,
                    "이미 사용중인 포트", "알림", JOptionPane.ERROR_MESSAGE);
        }
    ...
    class UserInfo extends Thread {
        ...
        private void UserNetwork() {
            try {
                ...
                //userID = dis.readUTF();
                st = new StringTokenizer(dis.readUTF(), "/"); // 준영: 고침
                userID = st.nextToken();
        ...
        public void run() { // 4100
            ...
            while (true) {
                try {
                    ...
                    recv_Msg(msg + "/" + msg + "/" + msg + "/" + msg); // 준영 : 슬래시 안넣으면 에러남 십팔
        ...
        public void recv_Msg(String str) {
            ...
            String protocol2 = st.nextToken(); //준영 : 비밀번호 기능을 위한 프로토콜
            String Message2 = st.nextToken(); //준영 : 비밀번호 기능을 위한 프로토콜
            ...
            else if (protocol.equals("CreateRoom"))
            {
                ...
                if (create_room_ok) // 방을 만들 수 있을 때
                {
                   ...
                   }else {         //준영 : 비밀번호가 설정된 경우
                      RoomInfo r = new RoomInfo(Message, Message2, this); //Message == RoomName, Message2 == 비밀번호 , 생성시에 room에 가입
                      room_vc.add(r); // 방을 추가
                      roomID = Message;
                      roomPW = Message2;
                       send_Msg("CreateRoom/" + Message);
            ...
            else if (protocol.equals("Join_Room"))
            {
                ...
                for (int i = 0; i < room_vc.size(); i++) {
                   ...
                   {    
                         if(r.Room_pw == "") {//준영 : 비밀번호가 없으면 바로 입장
                            if(is==null) {
                                try {
                            ...
                            break;
                       }else {   //준영 : 비밀번호가 있으면 비밀번호 요청
                             send_Msg("RequestPW/" + Message);
                        }
                    }
            ...
            else if (protocol.equals("Chatting"))
            {
                ...
                String chatting_msg = protocol2;
                for (int i = 0; i < room_vc.size(); i++) {
                    RoomInfo r = room_vc.elementAt(i);
                    if (r.Room_name.equals(Message)) // 해당 방을 찾으면
                    {
                        r.BroadCast_Room("Chatting/" + userID + "/" + chatting_msg);
                        System.out.println(chatting_msg);
                    }
                }
            }
            else if(protocol.equals("SendPW")) {   //준영 : 패스워드 체크 및 방입장
                for (int i = 0; i < room_vc.size(); i++) {
                    RoomInfo r = room_vc.elementAt(i);
                    if (r.Room_name.equals(Message2)) // 해당 방을 찾으면
                    {    
                          if(r.Room_pw.equals(Message)) {//준영 : 비밀번호 체크
                             if(is==null) {
                                 try {
                                is = socket.getInputStream();   // 영원: 방 입장시 채팅스트림 활성화
                                    dis = new DataInputStream(is);
                             ...
                             System.out.println(r.Room_user_vc.size());
                             roomID = Message2;
                             send_Msg("Join_Room/" + Message2); // Message(=방이름) 추가
                             break;
                        }else {//준영 : 비밀번호가 틀렸을 경우
                           send_Msg("DifferentPW/" + "/./.");
                         }
                     }
    ...
    class RoomInfo {
        ...
        RoomInfo(String name, UserInfo u) {            //준영 : 비밀번호가 없을 경우
            this.Room_name = name;
            this.Room_user_vc.add(u);
            System.out.println("비밀번호 설정안됨");
        }
        RoomInfo(String name, String pw, UserInfo u) {   //준영 : 비밀번호가 들어왔을 경우
            this.Room_name = name;
            this.Room_pw = pw;
            this.Room_user_vc.add(u);
            System.out.println("비밀번호 설정됨");
        }
